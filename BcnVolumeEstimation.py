...
class BcnVolumeEstimation(BcnWrapper):

    def __init__(self, connect_udp=True):
        """
        Constructor
            param connect_udp: connect_udp
        """
        # init
        # self.objpoints = []
        # self.imgpoints = []
    ...
    def get_ellipse_volume(self, value: str):

        split_value = value.split('?')
        result_path = str(split_value[0])
        resultList = []
        ...
        for i, profile in enumerate(profiles):
            ...
            print('DONE!')
            print(i)
            resultList.append([cortes[i],float(a)*float(b)*np.pi])
        ...
        if(EXPORT_CORTES):
            ...
            # find corte1 - Cruce con la media
            corte1 = None
            ...
            # find corte2 - máximo local en la diferencia
            corte2 = None
            max_area = 0
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        print(Fore.GREEN + arg + ', ' + Fore.RESET)

    t = BcnVolumeEstimation()
