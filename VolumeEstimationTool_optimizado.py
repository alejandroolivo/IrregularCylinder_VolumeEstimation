...
def split_pointcloud_in_profiles(pointcloud, tol=2, Z_MIN = -430, Z_MAX=1560, Y_MIN = -520, Y_MAX = 550):
    ...
    for i in range(len(pointcloud)):
        ...
        if prev_x is None or abs(x - prev_x) < tol:
            #Discard point if Z is out of Z_MIN and Z_MAX
            # if z < Z_MIN or z > Z_MAX or y < Y_MIN or y > Y_MAX or current_profile.__contains__([int(y), int(z)]):
            if current_profile.__contains__([int(y), int(z)]):
                continue
            
            # Add point to current profile
            current_profile.append([int(y), int(z)])
        else:
            # Start new profile
...
with open('F:/PointClouds/pts.xyz', 'r') as file:
    data = file.readlines()

# Modify the data by replacing commas with dots
modified_data = [line.replace(',', '.') for line in data]

# Open the file for writing and write the modified data back
with open('F:/PointClouds/pts.xyz', 'w') as file:
    file.writelines(modified_data)


# Load points from a .xyz file with ',' as decimal separator
pointcloud = np.loadtxt(r'F:\PointClouds\pts.xyz', delimiter=' ', usecols=(0, 1, 2), dtype=float)
# pointcloud = np.loadtxt(r'.\PointClouds\pts_test1.xyz', delimiter=' ', usecols=(0, 1, 2), dtype=float)
